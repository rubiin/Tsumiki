...
| **uv**                                           | `curl -LsSf https://astral.sh/uv/install.sh | sh`                                | Python package manager used by `./init.sh -setup` |
...
1. Run `./init.sh -start` from the project directory.
2. The Tsumiki bar should appear at the top of your screen.
3. If you see error messages, check that:
   - All system packages are installed (`./init.sh -setup` re-runs this check)
