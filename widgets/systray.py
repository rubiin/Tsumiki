...
class SystemTrayWidget(ButtonWidget):
    ...
    def __init__(self, widget_config: BarConfig, bar, **kwargs) -> None:
        ...
        self.children = Box(
            spacing=8,
            orientation=Gtk.Orientation.HORIZONTAL,
            children=(self.tray_box, self.toggle_icon)
        )

        # Create popup menu for ignored items
    ...
    def on_clicked(self, _):
        ...
        if visible:
            self.popup.hide()
            self.toggle_icon.set_from_icon_name("arrow-down-symbolic", 16)
            self.toggle_icon.get_style_context().remove_class("active")
        else:
            self.popup.show_all()
            self.toggle_icon.set_from_icon_name("arrow-up-symbolic", 16)
    ...
    def on_item_added(self, _, identifier: str):
        ...
        # Check if item should be ignored based on title
        ignored_list = self.config.get("ignored", [])
        is_ignored = any(x.lower() in title.lower() for x in ignored_list)
